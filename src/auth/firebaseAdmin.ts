...
const verifyIdToken = (token: string)=>{
    const firebasePrivateKey: string = process.env.FIREBASE_PRIVATE_KEY ?? "";//nullish coalescing - if null, return an empty string
...
return decoded.uid//return decoded user id
}; 
